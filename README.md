...
> **R:Porque el preprocesador incluye todo el contenido de los headers (como stdio.h) agregando muchas lineas al programa original**
...
> **R:con gcc -E programa.c | grep "Iniciando" no devuelve nada, con gcc -E -DDEBUG programa.c | grep "Iniciando" se devuelve printf("[DEBUG] %s\n", ("Iniciando main"));**
...
> **R:Indican el numero de linea y el archivo de origen del codigo. El bloque que contiene printf proviene de stdio.h**
...
> **R:Estas instrucciones marcan el inicio de la función sumar y preparan su entorno de ejecución: .LFB6 es una etiqueta interna que señala el comienzo del bloque, .cfi_startproc indica al compilador y al sistema de depuración que empieza una función y se activa el manejo del stack, endbr64 es una instrucción de seguridad del procesador que controla el flujo indirecto de ejecución, y pushq %rbp guarda el valor anterior del registro base para poder construir el stack frame de la función y así organizar correctamente sus variables y llamadas.**
...
> **R:En programa.o, area_circulo aparece como U (undefined) porque solo se la usa (se la llama) pero no está implementada en ese archivo, es decir, el compilador sabe que existe por su declaración pero no tiene su código. En cambio, en matematica.o aparece como T porque ahí sí está definida la función con su implementación real**
...
> **R:Quedan porque esas funciones están en una biblioteca externa (libc) y no dentro del programa.
Las resuelve el sistema operativo al ejecutar el programa, cargando la biblioteca y conectando esas funciones en ese momento.**
...
> **R:Una macro función como CUADRADO(x) se reemplaza directamente en el código en la etapa de preprocesamiento, antes de compilar. No existe como función real: el texto se “copia y pega” donde se usa. En cambio, una función como sumar(a, b) es una función real que pasa por todas las etapas del compilador (análisis, generación de código, etc.) y se traduce a ensamblador. La macro “desaparece” en el preprocesamiento, mientras que la función real llega hasta el enlazado. Además, la macro no tiene verificación de tipos, porque es solo sustitución de texto, mientras que la función sí la tiene en la etapa de compilación (análisis semántico).**
...
> **R:Un símbolo de tipo T en nm significa que es una función definida en la sección de código (.text), o sea instrucciones ejecutables. Un símbolo de tipo D significa una variable global inicializada, ubicada en la sección de datos (.data). En resumen: T vive en .text (código) y D vive en .data (datos inicializados en memoria).**
...
> **R:La salida de ldd o otool -L muestra las bibliotecas dinámicas que usa el programa, como libc.**

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:libc no se especifica explícitamente porque gcc la agrega automáticamente en el proceso de enlazado, ya que es la biblioteca estándar de C y todo programa C depende de ella por defecto (contiene printf, exit, malloc, etc.).**
